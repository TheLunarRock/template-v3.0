...
**最終更新**: 2026-04-26
**バージョン**: 3.7.3（CSP の `script-src` を環境別分岐: 開発時は `'unsafe-eval'` を追加して Next.js dev サーバーの React Refresh / HMR を動作可能に、本番ビルドでは従来通り `'unsafe-eval'` を含めずセキュリティ強化を維持。dev は localhost 限定で外部 XSS リスクなし）

...
デスクトップ通知は**気付くまで一定間隔で繰り返す**。macOS の通知は既定で数秒で消えるうえ、macOS 26 では通知スタイルを「持続的」に設定しても消えてしまい、「クリックするまで画面に残す」ことが OS 側の設定では実現できないためである（詳細と検証経緯は §11.10）。
...
| 通知は気付くまで繰り返す         | macOS 26 では通知スタイル「持続的」でも数秒で消える。OS 設定に依存しない方式を採る（§11.10） |
| ループはプロジェクト単位         | 複数リポジトリを並行で動かすため。1 つで操作を再開しても別リポジトリの通知は鳴り続ける       |
| Slack は繰り返さない             | スマホ側に履歴が残るため 1 回で足りる。繰り返すとチャンネルが埋まる                          |
...
| `CLAUDE_NOTIFY_INTERVAL`   | 繰り返しの間隔（秒）。既定 15                                            |
| `CLAUDE_NOTIFY_MAX`        | 繰り返しの上限回数。既定 10（上限到達で自然終了・鳴りっぱなし防止）      |
...
#### 11.10.1 なぜ「消えない通知」ではなく「繰り返し」なのか

要件は「作業が止まったことに確実に気付く」ことであり、当初は macOS の通知スタイルを**持続的**（クリックするまで画面に残る）にする方針で進めた。しかし 2026-08-31 に実機（macOS 26.6.2）で検証した結果、**この方法では実現できないと判明した**。試した経路と結果は以下のとおり。
...
結論として、**OS 側の設定に依存しない方式**を採る。通知が消えること自体は受け入れ、人間が操作するまで一定間隔で鳴らし続けることで「見逃さない」を達成する。

#### 11.10.2 仕様

| 項目       | 既定値         | 変更手段                                                    |
| ---------- | -------------- | ----------------------------------------------------------- |
| 間隔       | 15 秒          | `CLAUDE_NOTIFY_INTERVAL`                                    |
| 上限       | 10 回          | `CLAUDE_NOTIFY_MAX`（上限到達で自然終了・鳴りっぱなし防止） |
| 停止条件   | 操作の再開     | `PreToolUse` / `UserPromptSubmit` フック                    |
| 停止条件   | セッション終了 | 呼び出し元 `claude` プロセスの生存監視（§11.10.6）          |
| Slack      | 1 回のみ       | 繰り返さない（スマホ側に履歴が残るため）                    |
| 分離の単位 | プロジェクト   | ループはプロジェクトごとに独立                              |

#### 11.10.3 プロジェクト単位で分離する理由

複数リポジトリを並行で動かすため、ループは 1 本にまとめない。あるプロジェクトで承認しても、別プロジェクトで止まっている通知は鳴り続ける必要がある。
...
| `.claude/hooks/notify-repeat.sh` | 繰り返しループの起動・停止。キー生成を集約                                   |
| `.claude/hooks/notify-stop.sh`   | `PreToolUse` / `UserPromptSubmit` から呼ばれ、当該プロジェクトのループを停止 |
...
ループはプロセスグループごと停止するため、`sleep` 中の子プロセスも残らない。

#### 11.10.6 セッション終了時の扱い

ループはバックグラウンドプロセスとして起動するため、**親が終了しても生き残る**。停止フック（`PreToolUse` / `UserPromptSubmit`）は Claude Code のセッション内でしか発火しないので、エディタを閉じた後は誰もループを止められない。

この設計のまま運用したところ、**閉じたはずのエディタの通知が鳴り続ける事故が発生した**（2026-08-31）。複数リポジトリを開いていると、閉じる直前の完了通知が各プロジェクトで起動し、その数だけ鳴る。

対策として、呼び出し元の `claude` プロセスを監視する。

| タイミング     | 動作                                                                   |
| -------------- | ---------------------------------------------------------------------- |
| 起動前         | 監視対象が既に終了していればループを作らない（PID ファイルも作らない） |
| 各ループの先頭 | `kill -0` で生存確認し、終了していれば `break`                         |

監視対象の PID は `session_pid()` がプロセスツリーを遡って `claude` を探す。辿れなかった場合は**監視しない**（空を返す）。フック実行用のシェルは即座に終了するため、それを監視対象にするとループが 1 回で止まってしまうからである。
...
| 通知が鳴り止まない       | 停止フックが未登録、または別プロジェクトのループ | `bash .claude/hooks/notify-repeat.sh stop-all` で全停止。`ls ~/.claude/notify-repeat/` で稼働中を確認 |
| エディタを閉じた後も鳴る | §11.10.6 の生存監視が効いていない                | `stop-all` で停止。`session_pid()` が `claude` を辿れているか確認                                     |
| 通知がすぐ消える         | macOS 26 の仕様（§11.10.1）                      | 消えない通知は実現できない。繰り返しで気付かせる設計                                                  |

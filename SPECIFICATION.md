...
### 12.11 全自動開発設定（settings.local.json）
...
### 12.12 権限モデル（公開リポジトリ: template-v3.0）
...
| **7層防御との関係** | 7層防御がカバーしない領域（RLS未設定、認証チェック漏れ、NEXT*PUBLIC*誤用等）を補完  |
...
**注**: `security-reviewer` サブエージェントは削除済み（2026-03-30）。7層防御（gitleaks, GitHub Secret Scanning, Push Protection, Dependabot等）が自動で動作しており、手動サブエージェントの実用場面がなかったため。
...
| `/security`       | 7層防御                     | 補完   | 7層防御がカバーしない領域（RLS、認証チェック、入力検証等）を診断+実装     |

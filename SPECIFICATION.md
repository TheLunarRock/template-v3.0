...
| **8層防御との関係** | 8層防御がカバーしない領域（RLS未設定、認証チェック漏れ、NEXT*PUBLIC*誤用等）を補完  |
...
**注**: `security-reviewer` サブエージェントは削除済み（2026-03-30）。8層防御（gitleaks, GitHub Secret Scanning, Push Protection, Dependabot, セキュリティヘッダー等）が自動で動作しており、手動サブエージェントの実用場面がなかったため。
...
| `/security`       | 8層防御                     | 補完   | 8層防御がカバーしない領域（RLS、認証チェック、入力検証等）を診断+実装     |
| boundary-reviewer | `pnpm check:boundaries`     | 補完   | grepチェックに加え意味的分析を別コンテキストで実行                        |

**重要**: いずれの新機能も既存機能を**置き換えない**。既存のCLAUDE.mdルール、`pnpm check:boundaries`、8層防御、Git Hooksは全て従来通り動作する。

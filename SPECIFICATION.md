...
**歴史的経緯**: 以前のドキュメント（`SUPERCLAUDE_FINAL.md`）には「22コマンド」と記載されていたが、これは SuperClaude 本家の理論値であり、本テンプレート同梱の実測値は 18 である（PR運用モード切替コマンド `sc:enable-pr` / `sc:disable-pr` 追加により当初の 16 から増加）。`SUPERCLAUDE_V42_UPGRADE_PLAN.md` / `SUPERCLAUDE_V42_RISK_ANALYSIS.md` 内の「22」は 2025-10-25 時点の本家カウントとして保全している（歴史文書）。
...
### 15.4 Vercel チーム認証要件（v3.7.9 で追加・最重要）
...
### 15.8 過去のインシデント記録（学習履歴）

#### 15.8.1 2026-04-24: Build minutes 9時間到達事件

**経緯:**

- Dependabot 自動 PR が連続生成
- 各 PR が preview deploy をトリガー
- 同時に `claude-code-review.yml` も PR 毎に起動
- 二重消費で 1 日に Vercel build minutes 9 時間に到達

**対処（v3.7.7 / 4/29 commits）:**

- Dependabot 自動 security fix を無効化
- vercel.json で全 branch deploy 停止
- claude-code-review.yml を全プロジェクトから削除
- CLAUDE.md にブランチ remote push 禁止ルール追加

#### 15.8.2 2026-04-30: Vercel チーム認証で 22 プロジェクト同時障害

**経緯:**

- 4/29 の bulk update（vercel.json 適用）を `hr150131@gmail.com` author で全 22 リポに push
- Vercel team は `nrb29293@nifty.com` のみ verified email として認識
- 全 22 プロジェクトの最新 deploy が `Error`（build 0ms で拒否）
- 当初「Spend Cap Pause」と誤診断し、上限を $20→$30 に引き上げたが解消せず
- `vercel inspect` の `readyStateReason` 確認で真因が判明

**学び:**

- **真因特定は CLI の `readyStateReason` 確認が最速・最確実**
- UI の "Blocked" / "Error" 表示はカテゴリ分類が緩く、症状≠原因
- Vercel チーム認証は team-level の挙動なので 1 設定で全プロジェクトに影響

**対処（v3.7.9 / 本セクション追加）:**

- Vercel アカウントに `hr150131@gmail.com` を verified email として追加
- 22 プロジェクト全件 `vercel --prod --yes` で再 deploy → 全 Ready
- §15.4 として恒久的に文書化（再発時に即座に対処可能）
...
| 3   | `SETUP_GUIDE.md` L119           | 「`/sc:` コマンド体系: 22の統一コマンド」（v3.5.0 当時の実測 16・現行 18）                                                                                                                                                             | 実測値（当時 16・PR運用モード切替追加で現行 18）に修正、カテゴリ別内訳を追加                            |
| 4   | `SUPERCLAUDE_FINAL.md` L273-310 | 架空の 22コマンドテーブル（`sc:init` / `sc:security` / `sc:api-docs` / `sc:build` / `sc:deploy` / `sc:rollback` / `sc:metrics` / `sc:estimate` / `sc:save` / `sc:reflect` / `sc:load` / `sc:document` — 12個が `package.json` に不在） | 実測 16 のカテゴリ別テーブルに全面置換、各コマンドの実装を明記（その後 PR 運用モード切替追加で現行 18） |
...
**最終更新**: 2026-04-30
**バージョン**: 3.7.9（§15「デプロイ」を「アプリ再現レベル」に全面拡張: 9 サブセクション化（15.1 設計の全体像 / 15.2 開発フロー再現可能 / 15.3 vercel.json 仕様 / 15.4 Vercel チーム認証要件 / 15.5 コスト保護多層構造 / 15.6 ビルド設定 / 15.7 トラブルシューティング / 15.8 過去インシデント記録 / 15.9 設計判断）。2026-04-30 に発生した「Vercel チーム認証で 22 プロジェクト同時障害」を恒久文書化、`readyStateReason` による真因特定手順を明文化、GitHub Actions $0 budget × 4 カテゴリ・Vercel Spend Cap の正確な再現手順を追加）
...
| バージョン | 日付       | 変更内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **3.7.9**  | 2026-04-30 | §15「デプロイ」を「アプリ再現レベル」に全面拡張（8行 → 9 サブセクション・約350行）: (1) 15.1 デプロイ設計の全体像（5 つの絶対原則）、(2) 15.2 開発フロー再現可能レベル（branch dev → pnpm dev → main merge → push → Vercel 自動デプロイの完全手順）、(3) 15.3 vercel.json 仕様（`deploymentEnabled` セマンティクスと三層防御の役割）、(4) **15.4 Vercel チーム認証要件**（v3.7.9 で恒久文書化: `readyStateReason: no git user associated` の症状・原因・8ステップ対処手順）、(5) 15.5 コスト保護多層構造（GitHub Actions $0 budget × 4 カテゴリの正確な再現手順、Vercel Spend Cap = Pauses projects 設定）、(6) 15.6 ビルド設定、(7) 15.7 トラブルシューティング（CLI で `readyStateReason` 確認 → 5 種パターン別対処）、(8) 15.8 過去インシデント記録（4/24 Build minutes 9時間到達 / 4/30 22 プロジェクト同時障害の経緯と学び）、(9) 15.9 設計判断（8項目）。CLAUDE.md にデプロイ運用絶対ルール追加（chunked check / メール統合 / コスト保護維持） |
| **3.7.6**  | 2026-04-26 | 世界水準到達のための Phase A 改善3点: (1) `AGENTS.md` 新規追加で Cursor / GitHub Codex / Aider など他 AI ツール向け always-on ルール抜粋を提供（ベンダーロックイン解消）。(2) `tests/consistency/agents-sync.test.ts` 追加（2ケース）で AGENTS.md と CLAUDE.md の同期を自動検証—drift 発生時に修正方法を明示してコミットブロック。(3) 自己ルール `No Fake Metrics` 順守のため誇張数値を実測に修正: `pnpm sc:*` 16種 → 18種（PR 運用モード切替コマンド `sc:enable-pr` / `sc:disable-pr` 追加分を反映）、整合性テスト 39 → 8ファイル/45ケース。CLAUDE.md の `(維持)` / `(新規追加)` 接尾辞削除。`file-references.test.ts` / `command-references.test.ts` の検証対象に AGENTS.md を追加                                                                                                                                                                                                                                                                 |

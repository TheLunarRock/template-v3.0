...
Content-Security-Policy（CSP）はテンプレートに含めていない。

| 理由                            | 詳細                                                                         |
| ------------------------------- | ---------------------------------------------------------------------------- |
| プロジェクト固有性が高い        | 外部CDN、analytics、フォントサービス等の許可リストはプロジェクトごとに異なる |
| テンプレートでの厳格CSPは破壊的 | クローン後に外部リソースを追加するたびにCSP違反エラーが発生する              |
| 誤った安心感の防止              | 不完全なCSPは「設定済み」と誤認される危険がある                              |

**推奨:** クローン先プロジェクトで使用する外部リソースが確定した段階で、プロジェクト固有のCSPを設定する。

#### 12.9.4 クローン先でのカスタマイズ例

| ユースケース                   | 変更内容                                 |
| ------------------------------ | ---------------------------------------- |
| iframeで自サイトを埋め込みたい | `X-Frame-Options`を`SAMEORIGIN`に変更    |
| カメラ/マイクを使うアプリ      | `Permissions-Policy`から該当APIを除外    |
| CSPを追加したい                | `securityHeaders`配列にCSPヘッダーを追加 |

#### 12.9.5 動作確認方法

```bash
pnpm dev
curl -sI http://localhost:3000 | grep -E "^(X-Frame|X-Content|Referrer|X-DNS|Strict|Permissions)"
```
...
"pnpm": {
  "overrides": {
    ...
    "i18next-fs-backend@<2.6.4": "^2.6.5",
...
| パッケージ           | 対処日     | 脆弱性概要                                    | 脆弱バージョン              | パッチバージョン      | Advisory                                              | CVSS       |
| -------------------- | ---------- | --------------------------------------------- | --------------------------- | --------------------- | ----------------------------------------------------- | ---------- |
| `flatted`            | 2026-04-13 | Prototype Pollution via parse()               | `<=3.4.1`                   | `>=3.4.2`             | —                                                     | —          |
| `vite`               | 2026-04-13 | File system access control bypass             | `>=6.0.0 <=6.4.1`           | `>=6.4.2`             | —                                                     | —          |
| `minimatch`          | 2026-04-13 | ReDoS: wildcards / extglob / globstar         | `>=10.0.0 <10.2.3`          | `>=10.2.3`            | GHSA-3ppc-4f35-3m26 / 7r86-cg39-jmmj / 23c5-xmqv-rm74 | —          |
| `picomatch`          | 2026-04-13 | ReDoS: extglob quantifiers                    | `<2.3.2` / `>=4.0.0 <4.0.4` | `>=2.3.2` / `>=4.0.4` | GHSA-c2c7-rcm5-vvqj                                   | —          |
| `brace-expansion`    | 2026-04-13 | Zero-step sequence process hang               | 1.x / 2.x / 4.x の各範囲    | 各パッチ版            | —                                                     | —          |
| `yaml`               | 2026-04-13 | Stack overflow via deeply nested              | `>=2.0.0 <2.8.3`            | `>=2.8.3`             | —                                                     | —          |
| `i18next-fs-backend` | 2026-04-25 | Path traversal via unsanitised lng/ns         | `<2.6.4`                    | `>=2.6.5`             | GHSA-8847-338w-5hcj                                   | 8.2 (HIGH) |
| `postcss`            | 2026-04-25 | XSS via unescaped `</style>` in CSS Stringify | `<8.5.10`                   | `>=8.5.10`            | GHSA-qx2v-qp2m-jg93 / CVE-2026-41305                  | —          |
...
**最終更新**: 2026-04-25
**バージョン**: 3.7.1（第12.10章にHIGH脆弱性2件の対処を追加: `i18next-fs-backend` Path traversal (CVSS 8.2 / GHSA-8847-338w-5hcj) と `postcss` XSS via unescaped `</style>` (CVE-2026-41305 / GHSA-qx2v-qp2m-jg93) を `pnpm.overrides` で強制パッチアップグレード。`pnpm audit` クリーン状態を維持）

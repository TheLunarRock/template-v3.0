...
### 7.5 SuperClaude v4 統合コマンド（`pnpm sc:*` 18種）

**重要**: 本テンプレート同梱の `pnpm sc:*` スクリプト（**18種**）は、SuperClaude フレームワーク本家が提供するユーザーグローバルの `/sc:` スラッシュコマンド（`~/.claude/commands/sc/`）とは**別レイヤー**である。同名コマンドがあっても実装は独立しており、混同してはならない。
...
#### 🗂️ 18スクリプトの完全カタログ（`package.json` の `scripts` 実測値）
...
| コマンド             | 実装               | 用途                                                             |
| -------------------- | ------------------ | ---------------------------------------------------------------- |
| `pnpm sc:enable-pr`  | `sc-enable-pr.js`  | OFF → ON 切替（ブランチ保護適用＋`claude-code-review.yml` 復活） |
| `pnpm sc:disable-pr` | `sc-disable-pr.js` | ON → OFF 切替（ブランチ保護解除＋workflow削除）                  |
...
Claude Code が「作業を終えたとき」「人間の入力を待って止まっているとき」に、macOS 通知とサウンドで知らせる仕組み。クリップボードには触れない（§11.4.5）。
...
デスクトップ通知は**消すまで画面に残す**。`alerter` があれば通知を 1 回だけ出し、人間が消すまで待つ。`alerter` が無い環境では**気付くまで一定間隔で繰り返す**方式にフォールバックする。テンプレートは友人に配られるため `brew install` を前提にできないので、二経路を維持する（詳細と検証経緯は §11.10）。
...
└──────────────────┘
      ...
      │
┌──────────────────┐   ┌──────────────────────────────────────────┐
│ notify-stop.sh   │ ◀─│ PreToolUse / UserPromptSubmit フック      │
└──────────────────┘   │ （＝人間が操作を再開した）                │
                       └──────────────────────────────────────────┘
```
...
| 通知は消すまで画面に残す                   | `alerter` が消えるまで残り、消された理由を返す。無い環境は繰り返しにフォールバック（§11.10） |
| ループはプロジェクト単位                   | 複数リポジトリを並行で動かすため。1 つで操作を再開しても別リポジトリの通知は鳴り続ける       |
| Slack は 1 回だけ送る                      | スマホ側に履歴が残るため 1 回で足りる。繰り返すとチャンネルが埋まる                          |
| Stop の通知は遅らせる                      | `Stop` は 1 応答の終わりで発火する。auto mode では作業の途中で何度も鳴るため（§11.4.4）      |
| Slack もデスクトップ通知と同じジョブで送る | 遅延中にキャンセルされたとき「通知は取り消されたのに Slack だけ飛ぶ」を防ぐ（§11.4.4）       |
...
繰り返しを止める側も同じ `.claude/settings.json` に登録する。
...
`PreToolUse` は「承認して次のツールが動き出した」、`UserPromptSubmit` は「プロンプトを送った」ことを表す。どちらも人間が気付いた証拠になる。`PreToolUse` はツール実行のたびに走るため、`notify-stop.sh` に重い処理（node 起動など）を足してはならない。
...
| イベント         | 使用フィールド                                              |
| ---------------- | ----------------------------------------------------------- |
| **Stop**         | `hook_event_name` / `cwd`                                   |
| **Notification** | `hook_event_name` / `cwd` / `notification_type` / `message` |

`last_assistant_message` も渡ってくるが**使わない**（§11.4.5）。
...
#### 11.4.4 Stop の遅延とキャンセル

`Stop` は「タスクが完了した」ではなく**「アシスタントが 1 回の応答を終えた」**で発火する。Cursor の auto mode は 1 つの作業を何ターンにも分けて進めるため、そのままだと**作業の途中で何度も「作業が終わりました」が鳴る**。

「本当に止まったか」は発火時点では判定できない。次にツールが動くか、ユーザーがプロンプトを送るかを待って初めて分かる。そこで通知を遅らせ、その間に作業が再開されたらキャンセルする。

```
Stop を受け取る
  → すぐには通知しない。既定 15 秒待つ
  → 15 秒以内に PreToolUse / UserPromptSubmit が来たら → キャンセル（＝作業継続中）
  → 何も来なければ → 通知を出す（＝本当に止まった）
```

| 項目           | 挙動                                                                       |
| -------------- | -------------------------------------------------------------------------- |
| 対象           | `Stop` のみ                                                                |
| 既定の遅延     | 15 秒（`CLAUDE_NOTIFY_STOP_DELAY`。`0` で従来どおり即時）                  |
| 承認待ち       | **遅延させない。** 人間が動かないと進まない状態なので即時に鳴らす          |
| キャンセル経路 | `notify-stop.sh`（`PreToolUse` / `UserPromptSubmit`）                      |
| 連続する Stop  | 新しい `Stop` が来ると古い遅延ジョブは破棄される（二重に鳴らない）         |
| セッション終了 | 遅延中にエディタが閉じられたら破棄する（`WATCH_PID` の生存監視・§11.10.6） |

**キャンセルされたら Slack も送らない。** これは仕様であり、実装上の最重要要件である。デスクトップ通知だけ取り消されて Slack が飛ぶと、遅延させた意味が無くなる（Slack は履歴なので積み上がる）。

これを保証するため、**遅延・Slack 送信・デスクトップ通知を 1 本のバックグラウンドジョブにまとめている**。PID ファイルに入るのはこのジョブの PID だけなので、`notify-stop.sh` が kill すれば 3 つとも起こらない。`notify.sh` は Slack の宛先と本文を解決するだけで、送信そのものは行わない（`notify.sh` に `curl` は無い）。

| 段階                     | 実行者                                           |
| ------------------------ | ------------------------------------------------ |
| イベント判定・文言決定   | `notify.sh`                                      |
| Slack の宛先・本文の解決 | `notify.sh`（送信はしない）                      |
| 遅延 → Slack → 通知      | `notify-repeat.sh` のバックグラウンドジョブ 1 本 |

#### 11.4.5 クリップボードには触れない（2026-09-01 廃止）

**通知フックはクリップボードを一切操作しない。** かつては `Stop` の
`last_assistant_message` を `pbcopy` でクリップボードへ入れていたが、
**報告を `pbcopy` で受け渡す運用と衝突し、報告が上書きされるため廃止した（2026-09-01）。**

実際に起きていた順番:

```
1. 呼び出し側が報告本文を pbcopy        ← 人間が欲しいもの
2. ターンが終わる
3. Stop フックが発火 → last_assistant_message を pbcopy   ← 上書き
```

`last_assistant_message` は「報告をクリップボードにコピーしました」等のメタ文なので、
報告が失われてメタ文だけが貼られる。通知の利便性より害が大きいと判断した。

**この記録を消さないこと。** 理由が残っていないと、将来「通知から報告を読み返せると
便利」という理由で同じものが再び足される。報告の受け渡しは呼び出し側の責務である。
...
**任意:** `brew install vjeantet/tap/alerter` を入れると、通知が「消すまで画面に残る」方式に切り替わる（§11.10）。あわせて**システム設定 → 通知 → ターミナル**で「通知を許可」を ON、「通知スタイル」を「持続的」にする必要がある。未導入でも繰り返し方式で動作するため、`pnpm setup:sc` は導入を強制しない。

### 11.8 グローバル通知からの移行

v3.8.1 より前は `~/.claude/settings.json` にフックを書き込み、`~/.claude/slack-notify.sh` から通知していた。フックはユーザー設定とプロジェクト設定でマージされ、コマンド文字列が異なるものは別ハンドラとして**両方実行される**ため、旧設定が残っていると同じイベントで 2 回通知される。なお両方がテンプレートの `notify.sh` を起動する構成なら、§11.10.7 の排他により後発が先発を破棄して 1 回にまとまる。2 回通知されるのは、旧 `slack-notify.sh` のような**別の通知スクリプト**が残っている場合である。
...
通知の発行経路（`alerter` / フォールバック）は `tests/regression/2026-09-01-001-repeat-notification-cannot-be-dismissed.test.ts` が検証する（§11.10.6）。発火条件（待機では通知しない／承認待ちは通知する）と `CLAUDE_NOTIFY_NO_SLACK` は `tests/regression/2026-09-01-002-idle-prompt-floods-slack.test.ts` が検証する。`Stop` の遅延とキャンセル（キャンセル時に Slack も送らないこと・承認待ちは遅延しないこと・連続 Stop で二重に鳴らないこと）は `tests/regression/2026-09-01-003-stop-notification-fires-mid-task.test.ts` が検証する。同じプロジェクトの通知が同時に 2 回起動されても 1 回にまとまること（§11.10.7）は `tests/regression/2026-09-03-001-notify-concurrent-start-double-fires.test.ts` が検証する。
...
# 遅延中にキャンセル → 鳴らない
echo "{\"hook_event_name\":\"Stop\",\"cwd\":\"$PWD\"}" \
  | env CLAUDE_NOTIFY_NO_SLACK=1 CLAUDE_NOTIFY_STOP_DELAY=10 bash .claude/hooks/notify.sh
sleep 2 && bash .claude/hooks/notify-stop.sh
sleep 12 && ls ~/.claude/notify-repeat/   # 空・通知が出ていないこと

# 放置 → 遅延後に鳴る
echo "{\"hook_event_name\":\"Stop\",\"cwd\":\"$PWD\"}" \
  | env CLAUDE_NOTIFY_NO_SLACK=1 CLAUDE_NOTIFY_STOP_DELAY=5 bash .claude/hooks/notify.sh
sleep 8   # 通知が出ること
bash .claude/hooks/notify-stop.sh
```

上の「承認待ち」は `CLAUDE_NOTIFY_STOP_DELAY` を指定しても即時に鳴る（遅延の対象外）。

### 11.10 消すまで残る通知

#### 11.10.1 なぜ「繰り返し」から「消すまで残す」へ戻したのか

要件は「作業が止まったことに確実に気付く」ことである。方針は 2 段階で変わった。

**第 1 段階（2026-08-31 午前）— 持続的通知が実現できないと判明した。**
macOS の通知スタイルを**持続的**（クリックするまで画面に残る）にする方針で進めたが、実機（macOS 26.6.2）で以下のとおり全滅した。

| 経路                                | 通知の送信元                        | 結果                                                                 |
| ----------------------------------- | ----------------------------------- | -------------------------------------------------------------------- |
| 専用 `.app` を `osacompile` で作成  | 自作アプリ                          | ❌ `open` で起動するためフォアグラウンドを奪う。通知自体も出なかった |
| `osascript`（従来方式）             | `com.apple.scripteditor2`           | ❌ このアプリが通知設定の一覧に現れず、スタイルを変更する手段がない  |
| Cursor の通知スタイルを「持続的」に | ―                                   | ❌ 通知は Cursor 名義ではないため無関係だった                        |
| `terminal-notifier` 3.0.0           | `fr.julienxx.oss.terminal-notifier` | ❌ 許可オン・スタイル「持続的」に設定しても数秒で消えた              |

送信元は通知データベース（`~/Library/Group Containers/group.com.apple.usernoted/db2/db`）を直接照会して確定させた。プロセスの祖先が Cursor であっても、`osascript` の `display notification` は**スクリプトエディタ名義**で発行される点に注意（この誤認が原因で Cursor 側の設定変更を試し、空振りした）。

そこで OS 設定に依存しない「気付くまで一定間隔で鳴らし続ける」方式を採った（コミット `cf8a5ba`）。

**第 2 段階（2026-08-31 夕方）— 繰り返しが実運用で邪魔になった。**
作業完了のたびに最大 10 回 × 15 秒＝**2 分 30 秒**鳴り続け、しかも**バナーをバツ印で消しても止まらない**。`osascript` の `display notification` はディスミスをコールバックしないため、停止経路が「次のプロンプト送信」か「次のツール実行」しか無いことが原因である。「気付かせる」目的は達成したが、気付いた後に黙らせる手段が無いという別の実害が生まれた。

**第 3 段階（現行）— `alerter` で「消すまで残る 1 回の通知」にした。**
`alerter` は通知が消えるまでブロックし、**消された理由を返す**。これにより「気付くまで鳴らす」必要そのものが無くなった。

#### 11.10.2 alerter の実測結果

`alerter` 26.5（`brew install vjeantet/tap/alerter`）を実機で検証した結果。

| 検証項目                         | 結果                                                                           |
| -------------------------------- | ------------------------------------------------------------------------------ |
| 通知の残り方                     | ✅ 消すまで画面に残る（通知スタイルが「持続的」であること）                    |
| バツ印で消したとき               | ✅ `{ "activationType" : "closed" }` を返して終了                              |
| 放置したとき（`--timeout` 到達） | ✅ `{ "activationType" : "timeout" }` を返して終了                             |
| 別プロセスから消す               | ✅ `alerter --remove <group>` で通知が消え、**ブロック中のプロセスも終了する** |
| バックグラウンド起動             | ✅ `&` で起動しても通知は表示され、ブロックしたまま待機する                    |

`--remove` でブロック中のプロセスが終了することは、停止フックの設計上の前提である（PID を kill しても `alerter` が出したバナーは画面に残るため、`--remove` が必要）。

> **注意:** `alerter` を入れただけでは通知は表示されない。以下の 3 点がすべて必要で、いずれかが欠けると「出ない」または「出ても中身が読めない」状態になる（2026-08-31 に実際にこれで詰まった）。
>
> 1. **システム設定 → 通知** の上部「プレビューを表示」を「常に」または「ロックされていないときのみ表示」にする。「表示しない」だと本文が `1件の通知` に置き換わり、何の通知か分からない。**アプリ個別ではなく全体設定**のため見落としやすい
> 2. **システム設定 → 通知 → ターミナル**で「通知を許可」を ON にする
> 3. 同じく「通知スタイル」を「持続的」にする（バナーだと数秒で消える）
>
> `alerter` は `com.apple.Terminal` になりすまして通知を発行するため、**通知アプリ一覧に `alerter` という項目は現れない**。設定対象は「ターミナル」である。一覧に無いことを「許可されていない」と誤認しやすい（実際に誤認した）。

#### 11.10.3 二経路の仕様

`alerter` の有無で経路が分かれる。テンプレートは友人に配られるため `brew install` を前提にできず、フォールバックは必須である。

| 項目         | alerter 経路                                               | フォールバック経路                             |
| ------------ | ---------------------------------------------------------- | ---------------------------------------------- |
| 判定         | `command -v alerter`（`CLAUDE_NOTIFY_ALERTER` で上書き可） | 同左が失敗したとき                             |
| 発行回数     | 1 回だけ                                                   | 15 秒間隔・最大 10 回                          |
| 表示         | 消すまで画面に残る                                         | 数秒で消える（鳴らし直して気付かせる）         |
| 間隔・上限   | 使わない                                                   | `CLAUDE_NOTIFY_INTERVAL` / `CLAUDE_NOTIFY_MAX` |
| 保険         | `--timeout 1800`（取りこぼし時に永久に残さない）           | 上限回数で自然終了                             |
| バツ印で消す | ✅ 止まる                                                  | ❌ 止まらない（ディスミスを検知できない）      |
| Slack        | 1 回のみ                                                   | 1 回のみ                                       |
...
| 停止条件       | 経路                                               |
| -------------- | -------------------------------------------------- |
| 操作の再開     | `PreToolUse` / `UserPromptSubmit` フック           |
| セッション終了 | 呼び出し元 `claude` プロセスの生存監視（§11.10.6） |
| 手動           | `bash .claude/hooks/notify-repeat.sh stop-all`     |

**`alerter` は必ずバックグラウンドで起動する。** 消されるまでブロックするため、前景で呼ぶとフックが返らず Claude Code が停止する。

#### 11.10.4 プロジェクト単位で分離する理由

複数リポジトリを並行で動かすため、通知は 1 本にまとめない。あるプロジェクトで承認しても、別プロジェクトで止まっている通知は残す必要がある。

PID は `~/.claude/notify-repeat/<プロジェクト名>-<パスのハッシュ8桁>.pid` に置く。プロジェクト名だけでは同名リポジトリが衝突するためパスのハッシュを付ける。キー生成は `notify-repeat.sh` の `key_for()` に集約し、起動側（`notify.sh`）と停止側（`notify-stop.sh`）でズレないようにしている。

このキーは **`alerter` の `--group` にもそのまま渡す**。`--remove` で消す対象が常に「自分のプロジェクトの通知だけ」になり、`stop-all` では PID ファイル名（＝グループ ID）から復元して消せる。

#### 11.10.5 ファイル構成と手動での停止・確認

| ファイル                         | 役割                                                                       |
| -------------------------------- | -------------------------------------------------------------------------- |
| `.claude/hooks/notify.sh`        | イベント判定と文言決定。Slack の宛先解決。通知の発行は下記へ委譲           |
| `.claude/hooks/notify-repeat.sh` | 通知の発行と停止。経路の分岐・キー生成・セッション監視を集約               |
| `.claude/hooks/notify-stop.sh`   | `PreToolUse` / `UserPromptSubmit` から呼ばれ、当該プロジェクトの通知を消す |

```bash
# 出ている通知を一覧する
ls ~/.claude/notify-repeat/

# このプロジェクトの通知を消す
bash .claude/hooks/notify-stop.sh
```

`notify-stop.sh` は `PreToolUse` でツール実行のたびに走るため、PID ファイルが無ければ即座に返す（`alerter` の起動も行わない）。通知が出ているときだけ `alerter --remove` と PID kill の両方を実行する。

#### 11.10.6 セッション終了時の扱い

通知はバックグラウンドプロセスとして待機するため、**親が終了しても生き残る**。停止フック（`PreToolUse` / `UserPromptSubmit`）は Claude Code のセッション内でしか発火しないので、エディタを閉じた後は誰も止められない。

この設計のまま運用したところ、**閉じたはずのエディタの通知が鳴り続ける事故が発生した**（2026-08-31）。複数リポジトリを開いていると、閉じる直前の完了通知が各プロジェクトで起動し、その数だけ鳴る。

対策として、呼び出し元の `claude` プロセスを監視する。**この監視は `alerter` 経路でも必要である**（消すまで残る通知は、セッションが消えた後も画面に残り続けてしまうため）。

| タイミング            | 動作                                                                 |
| --------------------- | -------------------------------------------------------------------- |
| 起動前                | 監視対象が既に終了していれば通知を出さない（PID ファイルも作らない） |
| alerter 経路（5秒毎） | 終了していれば `alerter --remove` で通知を消し、待機プロセスを止める |
| 繰り返し経路（各周）  | `kill -0` で生存確認し、終了していれば `break`                       |

監視対象の PID は `session_pid()` がプロセスツリーを遡って `claude` を探す。辿れなかった場合は**監視しない**（空を返す）。フック実行用のシェルは即座に終了するため、それを監視対象にすると通知が 1 回で止まってしまうからである。

テストでは `CLAUDE_NOTIFY_WATCH_PID` で監視対象を上書きし、終了済みセッションを再現して検証する。
...
**回帰テスト:** `tests/regression/2026-09-01-001-repeat-notification-cannot-be-dismissed.test.ts` が二経路の分岐・`--group` の一意性・`--remove` の呼び出し・前景で呼ばないこと・`CLAUDE_NOTIFY_DRY_RUN` の出力不変を保証する。実機の `alerter` 有無や OS に依存しないよう、`uname` / `alerter` / `osascript` / `afplay` を `PATH` のスタブで差し替えて検証している。

#### 11.10.7 同時起動の排他（共通フックとプロジェクトフックの二重起動）

`~/.claude/settings.json`（全プロジェクト共通）とプロジェクトの `.claude/settings.json` の両方に通知フックがある環境では、1 回の `Stop` で `notify.sh` が 2 回起動される。`start` は先頭で `stop_loop` を呼んで古いジョブを消してから始めるが、2 つのプロセスが同時に走ると後発の `stop_loop` が先発の PID ファイル書き込みより先に走り、どちらも生き残る。実測でデスクトップ通知 2 回・Slack 2 通になった（2026-09-03）。共通フックとプロジェクトフックの併用は普通の構成なので、テンプレート側がこれに耐える。

| 項目         | 仕様                                                                                                 |
| ------------ | ---------------------------------------------------------------------------------------------------- |
| 排他区間     | `start` 経路の `stop_loop` から PID ファイル書き込みまで。バックグラウンドジョブ本体は区間の外で動く |
| ロック       | `~/.claude/notify-repeat/<キー>.lock` ディレクトリ。`mkdir` の原子性で取る（プロジェクト単位）       |
| 取れないとき | 0.1 秒間隔でリトライし、3 秒を超えたら**ロック無しで続行**する（通知を落とすより二重に出る方がまし） |
| 解放         | PID ファイル書き込みの直後。異常終了に備えて、取得より前に `trap … EXIT` を仕掛ける                  |
| 残骸         | 作成から 30 秒より古いロックは異常終了の残骸とみなして取り除く（作成時刻が取れない環境では触らない） |
| 対象外       | `stop` / `stop-all`。停止は何度走っても安全で、`PreToolUse` のたびに待たせたくない                   |

**回帰テスト:** `tests/regression/2026-09-03-001-notify-concurrent-start-double-fires.test.ts` が、同一プロジェクトの同時 2 起動で通知 1 回・Slack 1 通になること、別プロジェクトの同時起動ではそれぞれ 1 回ずつ出ること（ロックがプロジェクトをまたがない）、残骸のロックが取り除かれること、居座られたロックでも上限後に通知が出ることを保証する。競合の窓（`stop_loop` と PID ファイル書き込みの間で呼ばれる `uname`）をスタブで 0.5 秒遅らせ、二重起動を毎回確実に再現している。

### 11.11 トラブルシューティング

| 問題                       | 原因                                             | 解決策                                                                                                |
| -------------------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| 通知が2回来る              | 旧グローバル通知スクリプトが残っている           | §11.8 の移行手順を実施。同じ `notify.sh` の二重起動なら §11.10.7 の排他で 1 回にまとまる              |
...
| macOS 以外で通知が出ない   | 通知センター・`afplay` は macOS 前提             | Slack 連携のみ利用（macOS 依存部分は自動でスキップされる）                                            |
| CI で通知が飛ぶ            | `CI` 環境変数が設定されていない                  | CI 側で `CI=true` を設定、または `CLAUDE_NOTIFY_DISABLED=1`                                           |
| 通知が鳴り止まない         | 停止フックが未登録、または別プロジェクトの通知   | `bash .claude/hooks/notify-repeat.sh stop-all` で全停止。`ls ~/.claude/notify-repeat/` で稼働中を確認 |
| エディタを閉じた後も残る   | §11.10.6 の生存監視が効いていない                | `stop-all` で停止。`session_pid()` が `claude` を辿れているか確認                                     |
| 通知がすぐ消える           | `alerter` 未導入でフォールバック経路（§11.10.3） | `brew install vjeantet/tap/alerter`。導入後は消すまで残る                                             |
| alerter を入れても出ない   | ターミナルの通知が未許可／スタイルがバナー       | システム設定 → 通知 → ターミナル で「通知を許可」ON・「通知スタイル」を「持続的」に（§11.10.2）       |
| 本文が「1件の通知」になる  | 「プレビューを表示」が「表示しない」             | システム設定 → 通知 →「プレビューを表示」を「常に」／「ロックされていないときのみ表示」に（全体設定） |
| 通知一覧に alerter が無い  | `com.apple.Terminal` になりすまして発行するため  | 設定するのは「ターミナル」の側。一覧に現れないのが正常（§11.10.2）                                    |
| バツ印で消しても止まらない | `alerter` 未導入（繰り返しはディスミス非対応）   | `alerter` を導入する。暫定は `stop-all` で全停止                                                      |
| 繰り返しが 1 回で終わる    | `PreToolUse` が直後に発火して停止した            | 想定動作。ツールが動いた＝人間が承認したとみなしている                                                |
...
> **ローカルツールとの整合:** `pnpm sc:start` 等が呼ぶ `scripts/git-safety-check.js` も本モードを参照し、OFF では `main` を保護ブランチ扱いしない（main 直作業を許可）。仕様は §24.3.5 を参照。
...
| フェーズ | 内容                                                | リスク | 確認      |
| -------- | --------------------------------------------------- | ------ | --------- |
| **1**    | Secret Scanning + Dependabotアラート + 自動PR無効化 | なし   | Y/n       |
...
**フェーズ1は Dependabot 自動PRを「有効化しない」ではなく「明示的に無効化する」:**
`automated-security-fixes` は GitHub のデフォルトで有効になるため、DELETE を発行しないと
一括適用のたびに全リポジトリで自動PRが復活する（§15.5.3）。アラート（`vulnerability-alerts`）は
独立して有効のまま維持する。
...
| 環境別制御                         | dev では `'unsafe-eval'` を追加して HMR を許可、本番では除外                            |
...
#### 12.11.4 ガード迂回防止（2026-09-01 追加）

| denyルール                           | 防止する操作                     | 補足                                 |
| ------------------------------------ | -------------------------------- | ------------------------------------ |
| `mcp__serena__execute_shell_command` | ガードフックを通らないシェル実行 | serena の他機能は allow のまま使える |

**発見の経緯:** `permissions.allow` に `mcp__serena__execute_shell_command` が含まれていた一方、`PreToolUse` のガードフック（`git-push-guard` / `dev-server-guard` / `blocking-op-guard`）の matcher は `"Bash"` だけだった。**ガードフックも `deny` の `Bash(...)` 形式も、Bash ツールにしか反応しない。** そのため serena のシェル経由なら次がすべて警告もブロックも無く通っていた。

| 迂回できていた禁止事項         | 本来の防御                                  |
| ------------------------------ | ------------------------------------------- |
| `pnpm dev`                     | `dev-server-guard.sh`（exit 2 でブロック）  |
| `git push --force origin main` | `deny` + `ask` の両方に登録                 |
| `npx playwright`               | `blocking-op-guard.sh`（exit 2 でブロック） |

3層で守っているつもりのものが、**1 つの allow エントリで迂回できる状態**だった。

**対策は2つ。**

1. `allow` から削除し `deny` へ移す（本体）
2. `PreToolUse` の matcher を `Bash|mcp__.*__execute_shell_command` に広げる（保険）

2 は 1 で deny 済みのため現状は到達しないが、**将来別の MCP が同種のツールを持ち込んだときに自動で網にかかる**ようにするための多層防御である。ガードフックは `tool_input.command` を読むだけなので、パラメータ名が違う MCP に対しては空文字となり素通りする（誤発火しない）。

#### 12.11.5 allow の設計方針（コマンド名は境界にならない）

**`Bash(node:*)` は意図的に `allow` に残している。** コマンド名を allow に並べる方式は、任意コード実行（`node -e` / `sed -i` / `bash -c`）を許した時点で**実行内容の境界にはならない**。実効的に効いているのは「内容を検査して確定ブロックするガードフック」であり、`allow` の役割は毎回聞かれる煩わしさを減らすことに過ぎない。

`Bash(node:*)` を外しても `bash -c` で同じことができるため、**プロンプトが増えるだけで防御は増えない**。したがって allow から落とすべきなのは「任意コード実行だから危険」なものではなく、**ワークフローと矛盾するもの・使う場面が無いもの**に限る。

2026-09-01 に落とした項目:

| 項目               | 落とした理由                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------- |
| `Bash(git push:*)` | ルールは「AI はコミットまで。push は人間が指示する」。allow にあると機械的に守られない            |
| `Bash(curl:*)`     | 外部への送信・任意ダウンロードの経路。使う場面が無い（Slack 送信はフック内で permissions 対象外） |
| `Bash(npx:*)`      | ネットから任意パッケージを取得して実行できる。`npx playwright` で自動操作の禁止も破れる           |
| `Bash(rm:*)`       | `deny` は `rm -rf /` 等のリテラルしか見ておらず、任意ディレクトリを無確認で消せた                 |

`Bash(rm:*)` の削除に伴い、`ask` に `Bash(rm -rf:*)` と `Bash(rm -r:*)` を追加して**再帰削除は確認プロンプトを挟む**ようにした。`git push` も `allow` から外れたことで確認プロンプトになる（`ask` の `Bash(git push origin feature/*)` 等はそのまま）。

回帰テスト: `tests/regression/2026-09-01-005-serena-shell-bypasses-guards.test.ts`

#### 12.11.6 denyルールの解除方法
...
#### 12.15.7.1 追加の確定的enforcementフック（編集ブロック・push警告）

「禁止事項はプロンプト文でなく Hook / 権限で確定化する」方針（Anthropic 公式の steering ガイド準拠）に基づき、従来プロンプトのみだった既存ルールを `PreToolUse` フックで機械的に強制する。**ルール内容は変更せず、強制手段のみを追加**している。

##### (a) `.claude/hooks/protect-config-edit.sh` — 保護設定ファイルの編集ブロック

| 項目           | 仕様                                                                                                                                         |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **matcher**    | `Edit\|Write\|MultiEdit`                                                                                                                     |
| **目的**       | 「絶対に変更してはいけない設定ファイル」を **編集の瞬間に** 強制（従来は commit 時の `protect-config.js` のみ）                              |
| **保護リスト** | `scripts/protect-config.js` の `PROTECTED_FILES` を実行時に読む（単一の真実の源・フック側に複製しない）                                      |
| **exit code**  | 保護ファイル一致 → `exit 2`（stderr が Claude に渡りツール呼出しをブロック）／`.claude/settings.json` → 警告のみ `exit 0`／その他 → `exit 0` |
| **対人影響**   | Claude のツール経由編集のみブロック。人間がエディタで直接行う保守は妨げない                                                                  |
| **stdin/解析** | `{"tool_input": {"file_path": "..."}}` を node -e で解析（jq 非依存）                                                                        |

`db-destructive-warning.sh`（exit 0 固定）と異なり、本フックは **exit 2 でブロックする**点が設計上の違い。`.claude/settings.json` への編集は、deny/ask を弱める変更でないか注意喚起する警告のみ（保守自体は許可）。

動作確認（実ファイルを変更せずに判定を検証）:

```bash
# 保護ファイル → ブロック（exit 2）
echo '{"tool_name":"Edit","tool_input":{"file_path":"'"$PWD"'/tsconfig.json"}}' \
  | bash .claude/hooks/protect-config-edit.sh; echo "exit=$?"
# 通常ファイル → 通過（exit 0・無出力）
echo '{"tool_name":"Edit","tool_input":{"file_path":"'"$PWD"'/src/foo.ts"}}' \
  | bash .claude/hooks/protect-config-edit.sh; echo "exit=$?"
```

##### (b) `.claude/hooks/git-push-guard.sh` — git push 事故の警告

| 項目          | 仕様                                                                                                                                              |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **matcher**   | `Bash`                                                                                                                                            |
| **目的**      | 「feature branch をリモートに push しない」「force push は `--force-with-lease`」を行動の瞬間に提示                                               |
| **検出**      | (1) 現在ブランチが `feature/*` で origin への push、(2) コマンドが `origin .../feature/` を push、(3) 素の `--force`（`--force-with-lease` 以外） |
| **exit code** | 常に `exit 0`（警告表示のみ）。確認ゲートは `settings.json` の `ask` が担う二層防御                                                               |
| **背景**      | 過去に feature ブランチ連投 push で Vercel build minutes を大量消費した実害（silver-hp）                                                          |

##### (c) `settings.json` の `ask` 追加（git 事故防止系）

`Bash(git commit --no-verify:*)` / `Bash(git push --no-verify:*)` / `Bash(git push origin feature/*)` / `Bash(git push -u origin feature/*)` / `Bash(git push --set-upstream origin feature/*)` / `Bash(git push --force origin *)` を `ask` に追加。`deny` の `git push --force origin main/master` は最優先で維持されるため、本番ブランチへの force は引き続きブロックされる。

##### (d) パススコープ rule `.claude/rules/supabase-migrations.md`

`paths:` フロントマターで migration／SQL ファイルにスコープし、「`public` 新規テーブルへの明示 GRANT」ルール（§12.15.9・絶対ルール #12）を、該当ファイルを触る瞬間に確実に再提示する。内容は CLAUDE.md と同一（パススコープ版）。

##### `settings.json` でのHook登録（既存 DB Hook と併存）
    ...
    "PreToolUse": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash \"$CLAUDE_PROJECT_DIR/.claude/hooks/protect-config-edit.sh\""
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash \"$CLAUDE_PROJECT_DIR/.claude/hooks/git-push-guard.sh\""
          }
        ]
      }
    ]
...
#### 12.15.9 Data API 公開ポリシー（明示 GRANT・2026-05-30〜）

##### 12.15.9.1 背景と仕様変更の本質

Supabase の Data API（PostgREST / GraphQL / supabase-js）は、PostgreSQL の **テーブル権限（GRANT）** によってクライアントへの公開可否が決まる。従来の Supabase は新規プロジェクト作成時に `public` スキーマ全体へ `anon` / `authenticated` ロールの GRANT を自動付与していたため、`public` に作ったテーブルは作成と同時に API 経由でアクセス可能になっていた。

2026-05-30 の仕様変更により、**新規プロジェクトではこの自動 GRANT が廃止**された。結果として `public` スキーマに作成した新規テーブルは、明示的に `GRANT` を発行しない限り Data API から `404` / `permission denied` を返す。アプリ側では「テーブルは存在するのに supabase-js から空配列または権限エラーが返る」という症状で顕在化する。

重要な区別：

- **GRANT（テーブル権限）** = そのロールがテーブルへ「到達できるか」（API 公開の可否）
- **RLS（行レベルセキュリティ）** = 到達できたロールが「どの行を読み書きできるか」

両者は **AND 条件** で評価される。GRANT が無ければ RLS 以前にアクセス不可。GRANT があっても RLS が拒否すれば行は返らない。今回の変更は前者（到達可否）のデフォルトが「閉」に変わったというもので、後者の RLS の挙動は不変。

##### 12.15.9.2 ロールアウト日程

| 日付       | 適用範囲                                       | 対応要否                                   |
| ---------- | ---------------------------------------------- | ------------------------------------------ |
| 2026-05-30 | 全新規プロジェクトでデフォルト化               | 新規プロジェクトは初日から明示 GRANT 必須  |
| 2026-10-30 | 全既存プロジェクトの「新規テーブル」に強制適用 | 既存プロジェクトでも以降の新規作成は要対応 |

既存プロジェクトの **既存テーブルは公開状態が恒久的に維持**される（grandfathered）。猶予期間付きの段階移行ではなく、対象はあくまで「これから作るテーブル」。したがって **過去のマイグレーションを遡って書き換える必要はなく、実益もない**。本テンプレートの対応は「今後のテーブル作成フローに明示 GRANT を組み込む」ことに一本化する。

##### 12.15.9.3 テンプレート方針（`authenticated` + `service_role` のみ）

本テンプレートは 9 層防御のセキュリティ重視方針に従い、**デフォルトでは未認証ロール `anon` に GRANT しない**。理由は、未認証公開はデータ漏洩の最大の入口であり、「公開はオプトインで明示的に選ぶ」べきだから。付与先ロールの意味は以下。

| ロール          | 意味                                 | デフォルト GRANT |
| --------------- | ------------------------------------ | ---------------- |
| `anon`          | 未認証（ログインしていない）ユーザー | ❌ 付与しない    |
| `authenticated` | ログイン済みユーザー                 | ✅ 付与する      |
| `service_role`  | サーバーサイド（RLS をバイパス）     | ✅ 付与する      |

##### 12.15.9.4 標準マイグレーションテンプレート（再現用・完全版）

`public` にテーブルを作成するマイグレーションは、必ず **(1) テーブル作成 → (2) RLS 有効化 → (3) RLS ポリシー → (4) 明示 GRANT** の 4 ブロックを揃える。以下は認証ユーザー専用テーブル（各ユーザーが自分の行のみ操作）の完全な雛形。

```sql
-- ============================================================
-- migration: YYYYMMDDHHMMSS_create_todos.sql
-- 認証ユーザー専用テーブル（自分の行のみ参照・操作可能）
-- ============================================================
...
-- (4) Data API 公開のための明示 GRANT（authenticated + service_role のみ）
grant select, insert, update, delete on table public.todos to authenticated, service_role;
```

**シーケンス（連番列）を使うテーブルの場合:** `bigserial` / `serial` 列や `identity` 列を使う場合、INSERT 時にシーケンスへの権限が必要。テーブル GRANT に加えて以下を発行する。

```sql
-- このテーブルが使うシーケンスに対して（テーブル固有のシーケンスのみを対象にするのが安全）
grant usage, select on sequence public.<sequence_name> to authenticated, service_role;

-- スキーマ全体にまとめて付与する場合（既存シーケンスのみ対象。今後追加分は都度付与が必要）
-- grant usage, select on all sequences in schema public to authenticated, service_role;
```

**RPC（データベース関数）を Data API から呼ぶ場合:** 関数も同様に EXECUTE 権限の明示付与が必要。

```sql
grant execute on function public.<function_name>(<arg_types>) to authenticated, service_role;
```

##### 12.15.9.5 未認証公開が必要なテーブル（`anon` のオプトイン）

「ログイン不要で読ませる」要件があるテーブル（公開ランキング、公開記事一覧など）だけ、`anon` を明示的に追加する。**必ず RLS ポリシーとセット**で設計し、書き込みは `anon` に与えない（read-only に限定）。

```sql
-- 公開読み取り専用テーブルの例: public.leaderboard
...
-- 未認証ユーザーも「閲覧のみ」可能にする RLS ポリシー
create policy "leaderboard_public_read"
  on public.leaderboard for select
  to anon, authenticated
  using (true);

-- GRANT: anon には SELECT のみ。書き込みは authenticated / service_role に限定
grant select on table public.leaderboard to anon;
grant select, insert, update, delete on table public.leaderboard to authenticated, service_role;
```

##### 12.15.9.6 検証クエリ（Data API 公開状態の確認）

テーブルがどのロールに公開されているかは SQL で確認できる。`anon` が意図せず含まれていないかを必ずチェックする。

```sql
-- public スキーマのテーブルごとに、各ロールへ付与された権限を一覧
select
  table_name,
  grantee,
...
**ダッシュボードでの確認:** Supabase Dashboard → Advisors → Security Advisor で、Data API に公開されているテーブル・RLS 未設定テーブルを GUI でレビューできる。2026-10-30 の既存プロジェクト強制適用より前に一度レビューしておく。

##### 12.15.9.7 9 層防御・既存 DB 保護との関係

| 項目                   | 関係                                                                                   |
| ---------------------- | -------------------------------------------------------------------------------------- |
| **第9層 / RLS 補完**   | GRANT は「到達可否」、RLS は「行可否」。両者 AND で初めて安全な公開になる              |
| **12.15 DB 破壊保護**  | 本ポリシーは「公開範囲の最小化」であり、破壊操作保護（ask + Hook）とは独立した防御軸   |
| **`/security` スキル** | RLS 未設定・`anon` 過剰公開の検出を診断項目に含める（9 層がカバーしない領域の補完）    |
| **トークン最小権限**   | service_role キーはサーバーサイド限定。クライアントへ露出させない（NEXT*PUBLIC* 禁止） |

##### 12.15.9.8 Claude Code 実装時チェックリスト

`public` にテーブルを作成・追加するときは以下を満たすこと。

1. ☑ `create table public.<name>` の直後に `alter table ... enable row level security` を書いたか
2. ☑ 操作種別（select / insert / update / delete）ごとに RLS ポリシーを定義したか
3. ☑ `grant <privs> on table public.<name> to authenticated, service_role;` を書いたか
4. ☑ シーケンス／RPC を使うなら、それぞれに `grant usage,select on sequence` / `grant execute on function` を書いたか
5. ☑ `anon` への GRANT は「未認証公開が要件のテーブル」に限定し、read-only にしたか
6. ☑ 既存テーブルのマイグレーションは遡及変更していないか（不要・禁止）

##### 12.15.9.9 設計判断

| 項目                                         | 判断 | 理由                                                                            |
| -------------------------------------------- | ---- | ------------------------------------------------------------------------------- |
| **デフォルト GRANT を `authenticated` 中心** | 採用 | 未認証公開はオプトイン。9 層防御の「公開は明示的に選ぶ」原則と整合              |
| **既存マイグレーションを遡及変更しない**     | 採用 | 既存テーブルは公開維持。遡及書き換えは差分ノイズとリスクのみで実益ゼロ          |
| **RLS ポリシーをテンプレートに含める**       | 採用 | GRANT だけでは行制御がなく危険。GRANT + RLS をワンセットで提示し誤用を防ぐ      |
| **`all sequences in schema` をコメント扱い** | 採用 | スキーマ一括付与は今後追加分を取りこぼす。テーブル固有シーケンス指定を推奨      |
| **テンプレートに SQL 雛形を同梱しない**      | 採用 | 本テンプレートは Supabase 依存のみで supabase/ を持たない。仕様書の雛形を正典化 |

> 出典: Supabase changelog「新規プロジェクトでは `public` スキーマのテーブルがデフォルトで Data API に公開されなくなる」（2026-05-30 適用開始 / 既存プロジェクトは 2026-10-30 から新規テーブルに強制）

##### 12.15.9.10 Neon との線引き（このルールは Supabase 固有）

本節（12.15.9）の明示 GRANT ルールは **Supabase 固有**であり、Neon など「自動 REST API を持たない素の PostgreSQL ホスティング」には**該当しない**。両者はアクセスモデルが根本的に異なるため、DB を選んだ時点で適用ルールが変わる。

| 観点                                          | Supabase                                                                 | Neon（素の Postgres）                         |
| --------------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------- |
| クライアントからの到達経路                    | ブラウザ/アプリが Data API を直叩き（PostgREST / GraphQL / supabase-js） | バックエンド経由（接続文字列 + ORM/ドライバ） |
| `anon`/`authenticated` への GRANT で API 公開 | **ある（12.15.9 の論点）**                                               | **ない**（自動 API が存在しない）             |
| アクセス制御の主軸                            | RLS（行レベル）+ テーブル GRANT                                          | アプリ層（接続主体・認可ロジック）            |
| 2026-05-30 の Data API 仕様変更               | 影響あり                                                                 | **完全に無関係**                              |

**Claude Code 実装ルール:** Neon プロジェクトで `create table public.<name>` を書くとき、12.15.9 の `grant ... to anon, authenticated` や Supabase 流の RLS ポリシーを**機械的に流用しない**。Neon ではクライアントが DB に直接到達しないため、これらは不要かつ誤り。認可はアプリ層（API ルート／サーバーアクション）で実装する。

**DB 選択指針（テンプレート利用者向け）:**

| 要件                                                           | 推奨 DB  | 理由                                                     |
| -------------------------------------------------------------- | -------- | -------------------------------------------------------- |
| クライアント直叩き / 組み込み Auth / Realtime / Storage が要る | Supabase | Data API・認証・RLS が一体。フロント完結型に強い         |
| 素の PostgreSQL をバックエンド仲介で使う / serverless 接続     | Neon     | branching・autoscaling・scale-to-zero。API層は自前で持つ |

判断軸は「**クライアントが DB に直接到達するか（→ Supabase）／バックエンドが必ず仲介するか（→ Neon）**」。前者を選んだときだけ 12.15.9 の GRANT/RLS ルールが発動する。

##### 12.15.9.11 Neon 利用時のメタルール（個別仕様は追従しない）

Neon は変化が速く、破壊的変更・デフォルト変更が定期的に入る（実例: 2026-01 Auth SDK 全面書き換え、2026-05 Snapshot 課金導入、Postgres 拡張・API の段階的廃止）。これらは大半が「使う人だけ影響する」オプトイン、または使用時に即エラーになる fail-closed な変更であり、テンプレートが**個別の API 名・設定キーを先回りで文書化すると、使う頃には陳腐化している**。
...
このセクションは「アプリ再現レベル」の詳細仕様。新規プロジェクトで本テンプレートと同等のデプロイ運用を再現するために必要な全要素を網羅する。
...
# 4. ローカルでmain にマージ
git add -A
git commit -m "feat(foo): xxx"
git checkout main
git merge feature/foo
git branch -d feature/foo  # local branch 削除

# 5. main を push（これが唯一のリモート操作）
git push origin main
# → Vercel が自動的に production deploy をトリガー
# → ~50秒でビルド完了 → 本番反映

# 6. 動作確認（任意）
# 本番URL（例: https://timetable-the-lunar-rock-projects.vercel.app）にアクセス
```
...
**`deploymentEnabled` のセマンティクス:**

| キー     | 値      | 意味                                |
| -------- | ------- | ----------------------------------- |
| `"**"`   | `false` | **全 branch で auto-deploy を停止** |
| `"main"` | `true`  | main だけ例外的に auto-deploy 有効  |

**この二重指定が必要な理由:**

- Vercel のデフォルトは「全 branch で deploy 有効」
- `"**": false` だけだと main も止まる
- `"main": true` で main を例外として復活させる

**防御層としての役割:**

1. **第一の防御**: branch を remote に push しない運用（CLAUDE.md ブランチ運用ルール）
2. **第二の防御**: 仮にうっかり push しても vercel.json で deploy 停止
3. **第三の防御**: Dependabot 自動 PR が再有効化されても deploy しない

### 15.4 Vercel チーム認証要件（最重要）

**症状:** deploy が `Error` 状態になり、ビルド時間 `0ms`、`readyStateReason` が以下：

```
The Deployment was blocked because there was no git user associated with the commit.
```

**原因:** Vercel team は commit author email がチームメンバーの **verified email** と一致する場合のみ deploy を許可する。一致しない場合、build を開始する前に拒否する。

**確認方法:**

```bash
# 1. git commit author email を確認
git log --format='%ae' -3
# 例: hr150131@gmail.com

# 2. Vercel アカウントの verified email を確認
# https://vercel.com/account/settings/authentication
# → "Email" セクションに表示されているアドレス
# 例: nrb29293@nifty.com
...
Step 2: 「Email」セクション右の「Manage」ボタンをクリック

Step 3: 「Add Another」をクリック

Step 4: commit author email（git config user.email の値）を入力
        例: hr150131@gmail.com

Step 5: チェックボックス「I understand that this email will have access to my account」にチェック

Step 6: 「Save」をクリック → 確認メール送信

Step 7: 受信箱で「Email Verification」リンクをクリック
        → "Your email address was successfully authenticated."

Step 8: Vercel 再 deploy 実行
        cd <project-root>
        vercel --prod --yes
        → ~50秒で Ready
```

**1 アカウント、複数 email の設計思想:**

| 方式                                    | 採否     | 理由                                                                     |
| --------------------------------------- | -------- | ------------------------------------------------------------------------ |
| メール統合（同一アカウントに複数email） | **採用** | 単一 Vercel アカウントで複数の git author を許容、チームメンバー追加不要 |
| チームメンバー追加（別アカウント招待）  | 不採用   | アカウント分散・Owner 権限 1 つに集約できなくなる                        |
| Author email を Vercel メールに統一     | 不採用   | git config 変更・過去 commit の amend 必要・author 履歴破壊              |

### 15.5 コスト保護の多層構造（再現可能レベル）

#### 15.5.1 GitHub 予算設定（最優先・最強の防御）

**設定場所:** `https://github.com/settings/billing/spending_limit`

**全カテゴリ $0 + Stop usage: Yes:**

| カテゴリ   | Budget | Stop usage | 効果                                                     |
| ---------- | ------ | ---------- | -------------------------------------------------------- |
| Actions    | **$0** | Yes        | 無料枠を使い切った時点で停止（課金は発生しない）         |
| Codespaces | **$0** | Yes        | 同左                                                     |
| Packages   | **$0** | Yes        | 同左                                                     |
| Git LFS    | **$0** | Yes        | 同左                                                     |
| Sandbox    | **$0** | Yes        | 同左                                                     |
...
**この設定の意味:** 「無料枠（Actions: private 合計 2,000 min/month）を超えても課金は絶対に
発生しない、その代わり超過後はその月は使えなくなる」という最強の防御。予算カウンタが進むのは
無料枠を使い切った**後**の従量課金分だけであり、$0 は「初日からブロック」ではない。

**public リポジトリは対象外:** public リポジトリの Actions 実行は無料枠を消費しないため、
private 側が停止していても影響を受けない。

**停止したときに疑うべきは予算額ではなく消費元（2026-08 実測）:** 2026-08-21〜08-29 に
private リポジトリの workflow が起動を拒否され続けた。原因は $0 設定そのものではなく、
その月の無料枠 2,000 分を使い切っていたこと。約 1,600 分を Dependabot Updates が消費していた
（§15.5.3）。同じ $0 設定のまま 6〜7 月は CI が正常稼働しており（`fukui-system` の
2026-06-03〜08-03 の失敗実行 100 件に予算ブロックはゼロ、`panel-app` は 7 月に成功）、
8 月下旬だけ発生している。

したがって**対処は予算額の引き上げではなく、無料枠を食っている実行の削減**である。
予算を $0 から上げるには支払い方法の登録が必要になり、登録すると予算未設定かつ課金が
有効な製品が unlimited になるため、コスト保護としてはむしろ後退する。
...
or your spending limit needs to be increased` が出る場合はこの状態。
その月の無料枠が尽きているので、消費元を特定して減らす（Dependabot 自動 PR・cron・
不要な workflow）。翌月 1 日に無料枠がリセットされて自動復旧する。

**消費元の調べ方:**

```bash
# 予算ブロックされた実行の抽出（3〜5秒で failure = 起動拒否）
gh api "repos/<owner>/<repo>/actions/runs?per_page=100&status=failure" \
  --jq '.workflow_runs[] | "\(.created_at[0:10])|\((.updated_at|fromdate)-(.created_at|fromdate))"' \
  | awk -F'|' '$2+0 < 15 {print $1}' | sort | uniq -c

# 定期実行（cron）の消費確認
gh api "repos/<owner>/<repo>/actions/runs?per_page=100&event=schedule" --jq '.workflow_runs | length'
```

#### 15.5.2 Vercel Spend Cap

**設定場所:** `https://vercel.com/teams/<team>/~/settings/billing`

**推奨値:** Pro プランの場合 `$30`（$20 included credit + $10 buffer）

| 設定                    | 推奨値            | 理由                                 |
| ----------------------- | ----------------- | ------------------------------------ |
| Spend Limit             | `$30`             | $20 標準枠 + $10 バッファ            |
| Spend Management Action | `Pauses projects` | 上限到達で全プロジェクトを自動 Pause |

**Pause 後の挙動:** main への push が来ても deploy が `Blocked` になる。次サイクル（毎月リセット）で自動復活、または上限引き上げで手動復活。

#### 15.5.3 Dependabot 自動 PR 無効化
...
#### 15.5.4 claude-code-review.yml 不採用（PR運用 OFF時）

PR運用モード OFF のテンプレートデフォルトでは `claude-code-review.yml` を含まない。理由：

- PR 毎に Claude API を呼び出す重い workflow
- Build minutes と Claude API クレジットの双方を消費
- PR 運用そのものを行わない個人開発では不要

PR運用 ON に切り替え時のみ `pnpm sc:enable-pr` で復活させる。

### 15.6 ビルド設定（Vercel ダッシュボード）
...
| Node.js Version   | `.nvmrc` 参照（現状 20.x） |
| Production Branch | `main`                     |

### 15.7 デプロイ失敗時のトラブルシューティング

**Step 1: 真因の特定（最優先）**

CLI/UI 表示の "Error" / "Blocked" は症状であって原因ではない。**`readyStateReason` を確認すれば即座に原因が分かる:**

```bash
# 失敗した deployment の URL を取得
vercel ls 2>&1 | head -5

# 詳細取得
curl -s "https://api.vercel.com/v13/deployments/<dpl_id>?teamId=<team_id>" \
  -H "Authorization: Bearer $(cat ~/Library/Application\ Support/com.vercel.cli/auth.json | grep token | head -1 | cut -d'"' -f4)" \
  | python3 -m json.tool | grep -E '"readyState|"errorCode|"errorMessage'
```

**Step 2: 原因別対処**

| `readyStateReason` パターン                    | 原因                             | 対処                               |
| ---------------------------------------------- | -------------------------------- | ---------------------------------- |
| `no git user associated with the commit`       | §15.4 Vercel チーム認証          | email 追加＋verify                 |
| `Build was paused due to spend limit`          | §15.5.2 Spend Cap 到達           | 上限引き上げ or 翌月待ち           |
| `Vulnerable version of Next.js detected`       | Next.js 既知脆弱性               | `pnpm update next`                 |
| `Command "pnpm build" exited with 1`           | コードレベルのビルドエラー       | ローカルで `pnpm build` 再現＋修正 |
| `Deployment Disabled by Project Configuration` | §15.3 vercel.json で branch 停止 | 想定通り（main 以外なら正常）      |

**Step 3: build 自体は走った場合のログ確認**
...
| **GitHub 予算設定 全カテゴリ $0 + Stop usage**          | 採用 | 課金リスクの物理的ゼロ化。停止時は予算を上げず消費元を削る |
...
#### 17.2.6 `pnpm validate` への coverage 統合
...
### 18.1 Chrome 146 + GitHub Desktop プロトコルハンドラ問題
...
#### 24.3.5 PR_MODE_FLAG の消費（git-safety-check.js）

`scripts/git-safety-check.js`（`pnpm sc:start` / `sc:plan` / `sc:feature` / `sc:refactor` 等が呼ぶ Git 安全性チェック）は、このフラグを読み取って保護挙動を切り替える。これにより「OFF＝main 直 push」という運用ルール（CLAUDE.md §ブランチ運用）とツール挙動が一致する。**従来はフラグを無視して main を無条件ブロックしていたため、OFF モードの正規フローで `sc:start` が必ず失敗する不整合があった。これを解消した。**

| 関数                                       | 仕様                                                                                                                                                                                          |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getPrMode()`                              | `CLAUDE.md` を読み、§24.3.4 と同じ `PR_MODE_FLAG_START`〜`PR_MODE_FLAG_END` ブロック内の `PR運用モード**: (ON\|OFF)` を抽出。読取失敗・未検出時は `'OFF'`（テンプレート既定）にフォールバック |
| `isProtectedBranch(branch, prMode = 'ON')` | `prMode === 'OFF'` なら常に `false`（個人開発では main 直作業が正規フロー）。`ON` のとき `main` / `master` / `develop` / `production` を保護。第2引数省略時は後方互換で `ON` 扱い             |

OFF モードでは `pnpm sc:start` を main 上で実行しても合格し（`ℹ PR運用OFF（個人開発）: main での直接コミット・pushは許可されています` を表示して exit 0）、ON モードでは保護ブランチで `exit 1` し feature ブランチ作成を促す（`--auto-fix` 指定時は `feature/brainstorm-YYYY-MM-DD` を自動作成）。回帰テスト: `tests/regression/2026-06-22-001-sc-start-main-block-pr-mode.test.ts`（PR モード別の保護判定を固定）。
...
詳細な改訂履歴は `git log --oneline` を参照してください。

...
│   │   └── SKILL.md                       # バグ修正プロトコル
│   └── react-safety/
│       └── SKILL.md                       # React無限ループ防止パターン
└── agents/                                # 専門レビューエージェント（新規）
    ├── boundary-reviewer.md               # フィーチャー境界違反レビュー
    └── security-reviewer.md               # セキュリティ脆弱性レビュー
```
...
#### /react-safety — React無限ループ防止パターン

| 項目                  | 内容                                                                             |
| --------------------- | -------------------------------------------------------------------------------- |
| **ファイル**          | `.claude/skills/react-safety/SKILL.md`                                           |
| **呼出し方法**        | `/react-safety`                                                                  |
| **引数**              | なし（パターン集を読み込むのみ）                                                 |
| **CLAUDE.mdとの関係** | CLAUDE.mdの基本ルール（useMemo安定化等）に加え、詳細なパターン集を必要時のみ参照 |

**含まれるパターン:**

| パターン                   | 原因                               | 症状               | 修正方法                         |
| -------------------------- | ---------------------------------- | ------------------ | -------------------------------- |
| オブジェクト参照の不安定性 | 毎レンダリングで新オブジェクト生成 | API数百回/秒呼出し | `useMemo`で安定化                |
| 依存配列でのプロパティ参照 | `[config.category]`で変更検知失敗  | 無限ループ         | オブジェクト全体`[config]`を使用 |
| 不適切なuseCallback使用    | 関数が再作成され続ける             | 無限ループ         | useCallback削除、直接定義        |
| エラー時の状態更新ループ   | 空配列→エラー→再レンダリング       | 無限ループ         | 空配列は正常として扱う           |

**チェックリスト:**

- 依存配列の全オブジェクトが`useMemo`で安定化されているか
- `useRef`で前回値を追跡しているか（必要な場合）
- `useEffect`にクリーンアップ関数があるか
- `useEffect`内で条件なしに`setState`を呼んでいないか
- 空配列結果をエラーではなく正常として扱っているか
- `exhaustive-deps` ESLintルールを満たしているか
...
| **ツール**   | Read, Grep, Glob, Bash                          |
| **呼出し例** | 「サブエージェントでセキュリティレビューして」  |
|              | 「security-reviewerエージェントでチェックして」 |

**検査項目:**

| #   | 検査カテゴリ           | 検出対象                                          | 深刻度   |
| --- | ---------------------- | ------------------------------------------------- | -------- |
| 1   | ハードコード秘密情報   | APIキー、トークン、パスワード in ソースコード     | CRITICAL |
| 2   | インジェクション脆弱性 | SQL injection, XSS, Command injection             | CRITICAL |
| 3   | 認証・認可             | API routeの認証チェック漏れ、セッション管理不備   | HIGH     |
| 4   | データ露出             | クライアントサイドへの機密データ漏洩、console.log | HIGH     |
| 5   | 依存関係               | 既知の脆弱パッケージ、不要な依存                  | MEDIUM   |
| 6   | .gitignore準拠         | セキュリティパターンの欠落                        | MEDIUM   |
...
脆弱性がない場合: `"No security vulnerabilities found."`

**7層防御との関係:**

| 防御レイヤー                    | 役割     | security-reviewerとの関係    |
| ------------------------------- | -------- | ---------------------------- |
| .gitignore（第1層）             | 予防     | .gitignore準拠を検査         |
| gitleaks pre-commit（第2層）    | 予防     | ハードコード秘密情報を検査   |
| Claude Code denyルール（第4層） | 予防     | 補完（コード内脆弱性を検査） |
| security-reviewer               | **検査** | コード全体の脆弱性を分析     |

### 22.6 既存機能との共存マトリクス

| 新機能            | 既存機能                    | 関係性 | 説明                                                     |
| ----------------- | --------------------------- | ------ | -------------------------------------------------------- |
| `/fix-bug`        | CLAUDE.mdバグ修正プロトコル | 補完   | 同じルールを手順化して明示呼出し可能に。ルールは常時有効 |
| `/react-safety`   | CLAUDE.md無限ループ防止     | 補完   | 詳細パターン集を必要時のみ参照。基本ルールは常時有効     |
| boundary-reviewer | `pnpm check:boundaries`     | 補完   | grepチェックに加え意味的分析を別コンテキストで実行       |
| security-reviewer | 7層防御                     | 補完   | 予防レイヤーに加えコードレベルの検査を追加               |

**重要**: いずれの新機能も既存機能を**置き換えない**。既存のCLAUDE.mdルール、`pnpm check:boundaries`、7層防御、Git Hooksは全て従来通り動作する。

### 22.7 セッション検出と利用フロー

```
1. Claude Code起動
   → .claude/skills/ をスキャン → /fix-bug, /react-safety を登録
   → .claude/agents/ をスキャン → boundary-reviewer, security-reviewer を登録

...
#### 12.10.2 対処済み脆弱性（2026-04-13）

| パッケージ        | 脆弱性概要                            | 脆弱バージョン              | パッチバージョン      | Advisory                                              |
| ----------------- | ------------------------------------- | --------------------------- | --------------------- | ----------------------------------------------------- |
| `flatted`         | Prototype Pollution via parse()       | `<=3.4.1`                   | `>=3.4.2`             | —                                                     |
| `vite`            | File system access control bypass     | `>=6.0.0 <=6.4.1`           | `>=6.4.2`             | —                                                     |
| `minimatch`       | ReDoS: wildcards / extglob / globstar | `>=10.0.0 <10.2.3`          | `>=10.2.3`            | GHSA-3ppc-4f35-3m26 / 7r86-cg39-jmmj / 23c5-xmqv-rm74 |
| `picomatch`       | ReDoS: extglob quantifiers            | `<2.3.2` / `>=4.0.0 <4.0.4` | `>=2.3.2` / `>=4.0.4` | GHSA-c2c7-rcm5-vvqj                                   |
| `brace-expansion` | Zero-step sequence process hang       | 1.x / 2.x / 4.x の各範囲    | 各パッチ版            | —                                                     |
| `yaml`            | Stack overflow via deeply nested      | `>=2.0.0 <2.8.3`            | `>=2.8.3`             | —                                                     |
...
**最終更新**: 2026-04-15
**バージョン**: 3.5.0（第7章に SuperClaude v4 統合コマンド16種の完全カタログを追加・`pnpm sc:*` と Claude Code `/sc:` スラッシュコマンドの区別を明文化・整合性テスト40化）

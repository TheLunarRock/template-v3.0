...
ドキュメントと実装の乖離は AI の信頼性を破壊するため、`tests/consistency/` 配下の **8ファイル / 45ケース**で自動検証する。pre-commit と CI で自動実行される。

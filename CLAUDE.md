...
| スキル           | 呼出し方法              | 用途                                     |
| ---------------- | ----------------------- | ---------------------------------------- |
| **fix-bug**      | `/fix-bug [バグの説明]` | 回帰テストファースト手順を自動展開       |
| **react-safety** | `/react-safety`         | 無限ループ防止の詳細パターン集を読み込み |
...
| **security-reviewer** | 「サブエージェントでセキュリティレビューして」 | コードレベルの脆弱性検査         |

**既存機能との関係**: 全て補完であり置き換えではない。CLAUDE.mdルール・`pnpm check:boundaries`・7層防御・Git Hooksは従来通り動作する。

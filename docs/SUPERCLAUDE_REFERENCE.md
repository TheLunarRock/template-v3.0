...
| `CLAUDE_NOTIFY_MAX=10`      | 繰り返しの上限回数（上限到達で自然終了）         |
...
| 通知が鳴り止まない       | 停止フックが未登録、または別プロジェクトのループが動いている | `bash .claude/hooks/notify-repeat.sh stop-all` で全停止。`ls ~/.claude/notify-repeat/` で稼働中を確認 |
| エディタを閉じた後も鳴る | セッションの生存監視が効いていない                           | `stop-all` で停止。SPECIFICATION.md §11.10.6 を確認                                                   |
| 通知がすぐ消える         | macOS 26 の仕様。通知スタイル「持続的」でも消える            | 消えない通知は実現できないため、繰り返しで気付かせる設計にしている                                    |

...
> **📝 数値に関する注記**: 以下の「22コマンド」は2025-10-25当時のSuperClaude公式フレームワークの命令カウントであり、本テンプレート同梱の `pnpm sc:*` スクリプト（現行16種）とは別レイヤーです。当時の比較分析を保全するため、本文の数値は更新していません。現行の実測値は [SUPERCLAUDE_FINAL.md](../SUPERCLAUDE_FINAL.md) および `package.json` の `sc:*` を参照してください。
